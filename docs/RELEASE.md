...
`npm run tauri:ubuntu` 会使用 Cargo 锁文件构建，随后规范化依赖并解包校验安装树。产物 `Quota Float Ubuntu_<版本>_amd64.deb` 位于 `src-tauri/target/release/bundle/deb/`。标准化和验证脚本按当前版本选择 `.deb`，因此目录中可以安全保留 0.1.9、0.1.10 和 0.1.11 旧版测试包；构建 0.1.12 时应保持这些文件不变。
...
1. 使用 `sudo apt install "./Quota Float Ubuntu_0.1.12_amd64.deb"` 安装，确认依赖可解析；首次安装/更新扩展后注销并重新登录。
...
15. 校验 `sha256sum -c SHA256SUMS.txt`，确认 0.1.9、0.1.10 和 0.1.11 旧版 `.deb` 文件及校验和未变化。
